...
from octopus.core.gp_regressor import WeightedWhiteKernel, GaussianProcessRegressor
from octopus.preprocessing.image_processing import normalise


class GPEdgeTracer(object):
    """
    ...
    def __init__(self,
        ...
        -------------------
            ...
            value acts as a threshold to reduce score_thresh if the total number of new pixel candidates doesn't
            exceed the previous number of pixel candidates + pixel_thresh, i.e. N_{i+1} >= N_{i} + pixel_thresh.
        ...
        self.grad_img = normalise(grad_img, minmax_val=(0, 1), astyp=np.float64)
        ...
        # Set up global internal parameters using inputs.
        ...
        if isinstance(kernel_options, dict):
            ...
            if self.kernel_type == 'Matern':
                self.kernel_nu = kernel_options.get('nu', 2.5)
            elif self.kernel_type == 'ExpSineSquared':
                self.exp_sine_period = kernel_options.get('period', 1.0)
            elif self.kernel_type == 'RationalQuadratic':
                self.alpha = kernel_options.get('alpha', 1.0)

        # RBF or Matern or ExpSineSquared or RationalQuadratic
        else:
            kernel_type, sigmaf_opt, sigmal_opt = kernel_options
            self.kernel_type = ["RBF", "Matern", "ExpSineSquared", "RationalQuadratic"][int(kernel_type)]
            if self.kernel_type == "Matern":
                self.kernel_nu = [2.5, 1.5][int(kernel_type > 1)]
            elif self.kernel_type == "ExpSineSquared":
                self.exp_sine_period = self.edge_length / 4  # Default period
            elif self.kernel_type == "RationalQuadratic":
            ...
            # Length-scale (effective sinuosity)
        ...
        self.constant_kernel = gp.kernels.ConstantKernel(self.sigma_f,
                                                         constant_value_bounds="fixed")

        # Set up main GP kernel (RBF, Matern, ExpSineSquared, or RationalQuadratic)
        if self.kernel_type == 'Matern':
            self.gp_kernel = gp.kernels.Matern(nu=self.kernel_nu,
                                               length_scale=self.sigma_l,
                                               length_scale_bounds="fixed")
        elif self.kernel_type == 'ExpSineSquared':
            self.gp_kernel = gp.kernels.ExpSineSquared(length_scale=self.sigma_l,
                                                       periodicity=self.exp_sine_period,
                                                       length_scale_bounds="fixed",
                                                       periodicity_bounds="fixed")
        elif self.kernel_type == 'RationalQuadratic':
            self.gp_kernel = gp.kernels.RationalQuadratic(length_scale=self.sigma_l,
                                                          alpha=self.alpha,
                                                          length_scale_bounds="fixed",
                                                          alpha_bounds="fixed")
        else:
            self.gp_kernel = gp.kernels.RBF(length_scale=self.sigma_l,
                                            length_scale_bounds="fixed")
    ...
    def fit_predict_gp(self,
        ...
        # Set up observation noise for initial points and training/observed points
        alpha_init = np.ones((self.init.shape[0])) * 1e-7
        alpha_obs = np.ones((new_obs.shape[0] - self.init.shape[0]))
        alpha = np.concatenate([alpha_init, alpha_obs], axis=0)
        alpha = alpha[sort_obs]  # Apply sorting to alpha as well

        # Set up elements of kernel and optimisation parameters for GPR
        self.gp_params["random_state"] = seed
        noise_kernel = WeightedWhiteKernel(edge_length=self.edge_length,
                                           noise_weight=alpha,
        ...
        if not converged:
            ...
            self.gp_kernel.length_scale_bounds = (1e-5, 1e5)
    ...
    def kernel_density_estimate(self,
        ...
        # score is in [0,1].
        disc_kde = normalise(disc_kde, (0, 1), np.float64)
    ...
    def compute_new_obs(self,
        ...
        global bin_idx, best_pts_scores, unique_bins
        ...
        # to find more pixel coordinates with lower score. This check is ignored when there
        ...
        while (N_pixels - N_pixels_pre < self.pixel_thresh) and (N_pixels < self.algo_thresh):
            ...
            # Concatenate the best pixels and scores by thresholding scores. Ensure to switch columns of
    ...
    def __call__(self,
        ...
        ---------------
            ...
            show_init_post (bool, default False) : If flagged, show initial posterior predictive
                distribution (after fitting init ,and obs if present) and ask use to continue or not.
