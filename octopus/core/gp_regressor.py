...
from sklearn.base import BaseEstimator, RegressorMixin, MultiOutputMixin, clone
from sklearn.gaussian_process.kernels import RBF, ConstantKernel as C, Kernel, StationaryKernelMixin, Hyperparameter
...
class WeightedWhiteKernel(StationaryKernelMixin, Kernel):
    ...
    def __init__(self, noise_weight, edge_length,
                 noise_level=1.0, noise_level_bounds=(1e-5, 1e5)):
        self.noise_weight = noise_weight
        self.noise_level = noise_level
        self.noise_level_bounds = noise_level_bounds

        # ADDED - Using length of edge, determine if sampling or querying
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        # ADDED - Only add noise to observations, not to query points.
        if X.shape[0] == self.edge_length:
            if Y is not None:
                return np.zeros((X.shape[0], Y.shape[0]))
            else:
                self.noise_weight = np.zeros_like(X)
